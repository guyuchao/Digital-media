	...
	(function(){
		...
		function initUI(){
			...
			$("#tools_process").click(function () {
               ...
               /* $.post("/colorize/colorizer", {image : imgData}, function(data, status){
                    putImg('resImg',data);
                });*/

                $.ajax({
					url : '/seamcarving/seamcarver',
					type : 'post',
					async: true,//使用同步的方式,true为异步方式
					data : {image : imgData},//这里使用json对象
            ...
            //使用JQuery从后台获取JSON格式的数据
				$.ajax({
					type:"get",//请求方式
				   	url:"/seamcarving/seamcarver",//发送请求地址
			...
			$("#tools_save").click(function () {
				$.post("/save/objectremove",function (data) {
					if(data.code==200){
						alert("保存成功！");
					}
					else{
            ...
            $('input[name="file"]').fileupload({
                ...
                messages: {
                    ...
                    $.get("/files/objectremove",function(data){
